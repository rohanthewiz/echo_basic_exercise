...
var (
	jetDBTypes = map[string]string{`Age`: `integer`, `Color`: `text`, `CreatedAt`: `timestamp without time zone`, `ID`: `integer`, `Name`: `text`, `PilotID`: `integer`, `UpdatedAt`: `timestamp without time zone`}
	_          = bytes.MinRead
)
